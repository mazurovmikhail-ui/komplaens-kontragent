...
const r4 = parseContract('ДОГОВОР\nучастия в долевом строительстве № АС-12/2026\nООО СПЕЦИАЛИЗИРОВАННЫЙ ЗАСТРОЙЩИК "АРТЕМСТРОЙ-ДВ", именуемое в дальнейшем «Застройщик», и гражданин РФ Меек Иван Иванович, паспорт 05 00, ИНН 250204049273, именуемый «Участник долевого строительства». Цена 6 218 100 рублей. Средства вносятся на счёт эскроу в АО «Банк ДОМ.РФ», ИНН 7725038124. Застройщик: ИНН 2502062237 ОГРН 1202500010041');
ok(r4.parties.length === 2 && r4.parties.some(p=>p.inn==='250204049273' && /Меек/.test(p.name) && p.role==='Участник') && r4.parties.some(p=>p.inn==='2502062237' && p.role==='Застройщик') && /Банк/.test(r4.bankNote||'') && r4.num==='АС-12/2026' && r4.subj==='nedvizh' && r4.sum===6218100, 'S15 ДДУ: ' + JSON.stringify(r4.parties.map(p=>p.name+'/'+p.inn+'/'+p.role)) + r4.bankNote + r4.num + r4.subj);
